...
CFLAGS=-fno-pie -no-pie -m32 -g -ggdb -Wall -Werror -O0 \
  -fno-zero-initialized-in-bss -fno-stack-protector -ffreestanding


TASKIDLE=idle.tsk
TASKR1=taskRickC137.tsk
TASKM1=taskMortyC137.tsk
TASKR2=taskRickD248.tsk
TASKM2=taskMortyD248.tsk
TASKC=taskCronenberg.tsk

TASKS=tareas.tsk

TASKSBINS=$(TASKIDLE) $(TASKR1) $(TASKM1) $(TASKR2) $(TASKM2) $(TASKC)

CODESIZE=60928     # = 65536 - 0x1200 = 64KB - 0x1200
TASKSSIZE=45056    # = 2 * (4096) + 2 * (4096) + 2 * (4096) + 2 * (4096) + 2 * (4096) + 1 * (4096) = tareas + idle
KERNELSIZE=105984  # = CODESIZE + TASKSSIZE

TASKSTARTADDR=0x8000000
...
tasks: $(TASKSBINS)
	...
	$(QUIET) dd if=$(TASKR1)   bs=1 count=8K of=$(TASKS) seek=0K   status=noxfer > /dev/null 2>&1
	$(QUIET) dd if=$(TASKM1)   bs=1 count=8K of=$(TASKS) seek=8K   status=noxfer > /dev/null 2>&1
	$(QUIET) dd if=$(TASKR2)   bs=1 count=8K of=$(TASKS) seek=16K  status=noxfer > /dev/null 2>&1
	$(QUIET) dd if=$(TASKM2)   bs=1 count=8K of=$(TASKS) seek=24K  status=noxfer > /dev/null 2>&1
	$(QUIET) dd if=$(TASKC)    bs=1 count=8K of=$(TASKS) seek=32K  status=noxfer > /dev/null 2>&1
	$(QUIET) dd if=$(TASKIDLE) bs=1 count=4K of=$(TASKS) seek=40K  status=noxfer > /dev/null 2>&1
	@echo ''
