...
#-------------------------------------------------------------------------------
# Este script genera los resultados para el experimento 3 del informe
...
cp Zigzag_asm_exp2.asm .temp
mv Zigzag_asm_exp2.asm Zigzag_asm.asm
...
mkdir resultados/exp3 2> /dev/null
mkdir resultados/exp3/original 2> /dev/null
mkdir resultados/exp3/modded 2> /dev/null
...
do
	...
	mv log.Zigzag.asm.out "resultados/exp3/modded/log.exp3.asm.${i}.out"
	mv log.Zigzag.asm.csv "resultados/exp3/modded/log.exp3.asm.${i}.csv"
done
...
do
	echo "+ IMG actual: ${IMGPREFIX}${i}"
	./filter_ejec.sh Zigzag asm $CANT_ITER "${IMGPREFIX}${i}"
	mv log.Zigzag.asm.out "resultados/exp3/original/log.exp3.asm.${i}.out"
	mv log.Zigzag.asm.csv "resultados/exp3/original/log.exp3.asm.${i}.csv"
done
